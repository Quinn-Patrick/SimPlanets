...
public class DragableCamera : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        float zoomSquared = -(Screen.height * 0.8657314629f) / _zoomLevel;
        transform.position += new Vector3(delta.x / zoomSquared , delta.y / zoomSquared, 0f);
    ...
    private void OnGUI()
    {
        GUI.Label(new Rect(10, 10, 1000, 20), $"Screen Resolution: {Screen.width} x {Screen.height}");
    }
}